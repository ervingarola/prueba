...
def insertar_producto(id_producto, nombre, fecha_ingreso, fecha_vencimiento):
    db = conectar_db()###### mandamos a llamar la funcion conectar bd con la variable
    cursor = db.cursor()###### se crea un objeto para ejcutar comando del sql
    sql = "INSERT INTO productos (id_producto, nombre, fecha_ingreso, fecha_vencimiento) VALUES (%s, %s, %s, %s)"
    val = (id_producto, nombre, fecha_ingreso, fecha_vencimiento)###### se crea una tupla q organiza los datos 
    cursor.execute(sql, val) ###### nos sirve par hacer una consulta
    db.commit()###### commit es para confirma 
...
def buscar_producto(nombre):
    ...
    resultado = cursor.fetchall()###### es un obeto ocupado para las consultas para devolver los datos del registro
...
def menu():
    while True: ###### se utiliza para crear bloques de codigos de bucles
        ...
        print("8. Salir")
        opcion = input("ingrese una opción:\n ")###### el input sirve para ingreesar una linea d comandos

        if opcion == "1":###### utlizamos el if para condicionar 
